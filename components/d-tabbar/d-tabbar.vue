<template>
  <view v-if="show" class="tn-tabbar-class tn-tabbar " @touchmove.stop.prevent="() => {}">
    <!-- tabbar 内容-->
    <view
      class="tn-tabbar__content solid-top"
      :class="{
...
<script>
	/**
	 * 底部导航栏组件  
	 * @property {Boolean} show 显示底部导航栏 - 默认true 
	 * @property {Array} list 底部导航栏信息配置- Array
	 * @property {String} iconSize 图标大小，单位rpx 
	 * @property {Number} fontSize 字体大小，单位rpx
	 * @property {Number} height 导航栏高度，单位rpx 
	 * @property {Number} outHeight 突起按钮的高度，单位rpx
	 * @property {String} bgColor 背景颜色
	 * @property {String} activeColor 激活时文字的颜色  
	 * @property {String} inactiveColor 未激活时文字的颜色  
	 * @property {String} activeIconColor 激活时图标的颜色，可以使用TuniaoUI内置的颜色 
	 * @property {String} inactiveIconColor 未激活时图标的颜色，可以使用TuniaoUI内置的颜色
	 * @property {Object} activeStyle 激活时自定义样式  Object
	 * @property {Boolean} shadow 显示阴影
	 * @property {Boolean} animation 点击时触发图标动画  
	 * @property {String} animationMode 触发的动画模式
	 * @property {Boolean} fixed 固定在底部
	 * @property {Boolean} safeAreaInsetBottom 开启底部安全区适配，开启的话，会在iPhoneX机型底部添
	 * @property {Boolean} beforeSwitch 切换前触发钩子函数
	 * @event {Function} change 切换导航栏选项时触发  回调参数：index：当前选中的选项序号
	 * @example 
	 */
	import colorStyle from "@/config/color.js"
	// console.log(colorStyle,'getFontColorStyle');
  export default {
    name: 'd-tabbar',
    props: {
      // 绑定当前被选中的current值
    ...
    computed: {
      ...
      elColor() {
        return (index, style = true) => {
          ...
          // 判断是否获取内部样式
          if (style) {
            if (colorStyle.getFontColorStyle(color) !== '') {
              return color
            } else {
              return ''
            }
          ...
          if (style) {
            if (colorStyle.getFontColorStyle(color) !== '') {
              ...
              return ''
						...
						// if(this.activeIconColor!='#01BEFF'){
						// 	 return color
						// }else{
						// 	return '#01BEFF'
						// }
      ...
      itemButtonClass() {
        return (index) => {
          ...
          if (this.list[index]['out']) {
            ...
            if (colorStyle.getFontColorStyle(this.activeIconColor) === '') {
